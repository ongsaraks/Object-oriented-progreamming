...
public class Playlist {
	...
	public ArrayList<Song> getSongOnly(){
		// YOUR CODE HERE
		ArrayList<Song> Agc = new ArrayList<Song>();
		for(int i = 0; i < list.size();i++) {
			if(list.get(i) instanceof Song) {
				Agc.add((Song)list.get(i));
			}
		}
		return Agc;
		// ------------------------------------
	...
	public ArrayList<Podcast> getPodcastOnly(){
		// YOUR CODE HERE
		ArrayList<Podcast> Agp = new ArrayList<Podcast>();
		for(int i = 0; i < list.size();i++) {
			if(list.get(i) instanceof Podcast) {
				Agp.add((Podcast)list.get(i));
			}
		}
		return Agp;
		// ------------------------------------
	...
	public int removeAudio(int opt) {
		...
		else if(opt == 2 && list.get(i) instanceof Song) {
			list.remove(i);
			crm++;
		}
		else if(opt == 3 && list.get(i) instanceof Podcast) {
			list.remove(i);
			crm++;
		}
		}
